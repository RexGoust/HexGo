...
mod tests {
    ...
    fn gnn_forward_produces_expected_output_shapes() {
        ...
        let model = GnnModel::<CpuBackend>::new(config, &device);

        let x = Tensor::<CpuBackend, 2>::zeros([88, FEATURE_DIM], &device).unsqueeze::<3>();
        let adj = Tensor::<CpuBackend, 2>::zeros([88, 88], &device);
